...
<article class="page-container">
	...
	<p class="short-input ue-clear">
    	<label>标题：</label>
        <input name="title" id="title" type="text" value="<?php echo ($data["title"]); ?>" placeholder="标题..." />
    </p>
    <p class="short-input ue-clear" style="margin-top: 10px;">
    	<label>作者：</label>
        <input name="author" type="text" id="author" value="<?php echo ($data["author"]); ?>" placeholder="作者..." />
    </p>
    <p class="short-input ue-clear" style="margin-top: 10px;">
    	<label>内容：</label>
        <script type="text/plain" id="editor" name="content" style="width: 800px;height: 300px;"><?php echo (htmlspecialchars_decode($data["content"])); ?></script>
    </p>
	</div>
...
$('.tj').click(function(){
   ...
   $.ajax({url:"<?php echo U('add');?>",type:"post",data:{'id':id,'title':title,'author':author,'content':content},success:function(res){
                if(res.code == 1){
                    layer.msg(res.msg, {
                      icon: 1,
