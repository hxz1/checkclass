...
<article class="page-container">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="<?php echo ($data["name"]); ?>" data-id="<?php echo ($data["id"]); ?>" placeholder="请输入用户名" id="name" name="name">
		</div>
	</div>
...
<script type="text/javascript">
	$('.tj').click(function(){
		...
		var phone = $('#phone').val();
		var email = $('#email').val();
		$.ajax({url:"<?php echo U('edit');?>",type:"post",data:{'id':id,'name':name,'sex':sex,'phone':phone,'email':email},success:function(res){
		 		if(res.code == 1){
		 			layer.msg('修改成功！', {
					  icon: 1,
