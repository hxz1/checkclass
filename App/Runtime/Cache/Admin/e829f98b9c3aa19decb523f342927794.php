...
<article class="page-container">
	...
	<div class="row cl">
		...
		<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>分院名：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="<?php echo ($data["name"]); ?>" id="name">
		</div>
	</div>
	...
	<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>最大选课数：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="number" class="input-text" value="<?php echo ($data["maxnums"]); ?>" id="num">
		</div>
	</div>
	<div class="row cl">
...
<script type="text/javascript">
	$('.tj').click(function(){
		...
		var name = $('#name').val();
		var num = $('#num').val();
		$.ajax({url:"<?php echo U('edit');?>",type:"post",data:{'id':id,'name':name,'maxnums':num},success:function(res){
		 		if(res.code == 1){
		 			layer.msg('修改成功！', {
					  icon: 1,
