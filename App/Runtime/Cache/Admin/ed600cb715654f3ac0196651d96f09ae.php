...
<article class="page-container">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="<?php echo ($data["teach_id"]); ?>" data-id="<?php echo ($data["id"]); ?>"  id="name" name="name">
		</div>
	</div>
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="<?php echo ($data["realname"]); ?>" placeholder="请输入真实姓名" id="realname" name="realname">
		</div>
	</div>
...
<script type="text/javascript">
	$('.tj').click(function(){
		...
		var email = $('#email').val();
		$.ajax({url:"<?php echo U('edit');?>",type:"post",data:{'id':id,'teach_id':tea_id,'sex':sex,'realname':realname,'email':email},success:function(res){
		 		if(res.code == 1){
		 			layer.msg('修改成功！', {
					  icon: 1,
